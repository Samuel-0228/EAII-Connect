  ...
  if (submitted) {
    return (
      <div className="max-w-xl mx-auto py-20 px-4 text-center">
        ...
        <h2 className="text-3xl font-bold text-slate-900 mb-4">{lang === 'en' ? 'Thank You!' : 'እናመሰግናለን!'}</h2>
        <p className="text-slate-600 mb-8">{lang === 'en' ? 'Your contribution has been recorded and will be verified.' : 'ተሳትፎዎ ተመዝግቧል፣ በቅርቡ ይረጋገጣል።'}</p>
        <button 
          onClick={() => setSubmitted(false)}
          className="bg-[#00885a] text-white px-8 py-3 rounded-lg font-bold"
        >
          {lang === 'en' ? 'Contribute More' : 'ተጨማሪ ይሳተፉ'}
        </button>
      </div>
    );
  }
        ...
        <div className="grid lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="flex bg-slate-100 p-1 rounded-xl">
              ...
              >
                <Camera size={18} /> <span>{lang === 'en' ? 'Upload' : 'መላክ'}</span>
              </button>
              <button 
              ...
              >
                <Tag size={18} /> <span>{lang === 'en' ? 'Label' : 'መሰየም'}</span>
              </button>
            </div>
            ...
            {activeTask === 'upload' ? (
              <div className="bg-white p-10 rounded-3xl border border-slate-200 text-center animate-in zoom-in duration-300">
                ...
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 mb-8">
                  ...
                  <button 
                    ...
                    className={`flex items-center space-x-2 px-4 py-2 rounded-full font-bold text-sm transition-all ${isRecording ? 'bg-red-500 text-white shadow-lg' : 'bg-white border text-slate-700 hover:bg-slate-50'}`}
                  >
                    {isRecording ? <MicOff size={16} /> : <Mic size={16} />}
                    <span>{isRecording ? (lang === 'en' ? 'Stop Recording' : 'መቅዳት ያቁሙ') : (lang === 'en' ? 'Record Voice' : 'ድምጽ ይቅዱ')}</span>
                  </button>
                </div>
              ...
              <div className="bg-white p-6 rounded-3xl border border-slate-200 animate-in zoom-in duration-300">
                ...
                <div className="mt-8 pt-8 border-t">
                  <button 
                    onClick={() => setIsRecording(!isRecording)}
                    className={`flex items-center space-x-2 px-4 py-2 rounded-full font-bold text-sm transition-all ${isRecording ? 'bg-red-500 text-white' : 'bg-slate-100 text-slate-700'}`}
                  >
                    <Mic size={16} />
                    <span>{lang === 'en' ? 'Voice Label' : 'በድምፅ ሰይም'}</span>
                  </button>
                </div>

                <div className="mt-8 flex items-center justify-between">
                  <p className="text-sm text-slate-500"><button className="underline">Skip</button></p>
                  <button onClick={handleSubmit} className="bg-[#00885a] text-white px-8 py-3 rounded-xl font-bold">{lang === 'en' ? 'Next' : 'ቀጣይ'}</button>
                </div>
              </div>
            )}
          </div>
      ...
      {showLoginPrompt && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-3xl p-8 text-center animate-in zoom-in duration-300">
            ...
            <p className="text-slate-500 text-sm mb-8">{lang === 'en' ? 'Please login to record your contributions and earn points.' : 'እባክዎ ተሳትፎዎ እንዲመዘገብ እና ነጥብ ለማግኘት ይግቡ።'}</p>
            <div className="space-y-3">
              <button 
                onClick={() => onNavigate('#login')}
                className="w-full bg-[#00885a] text-white py-4 rounded-xl font-bold flex items-center justify-center space-x-2"
              >
