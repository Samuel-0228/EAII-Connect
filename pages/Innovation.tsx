    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
        ...
        <button className="bg-[#00885a] text-white px-6 py-3 rounded-xl font-bold flex items-center shadow-lg hover:scale-105 transition-transform">
          <Plus size={20} className="mr-2" /> {lang === 'en' ? 'Post Idea' : 'ሀሳብ ያቅርቡ'}
        </button>
      </div>
      ...
      <div className="grid lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
           <div className="bg-white p-6 rounded-2xl border border-slate-200">
             <h3 className="font-bold mb-4 flex items-center"><Filter size={16} className="mr-2" /> {lang === 'en' ? 'Filters' : 'ማጣሪያዎች'}</h3>
             <div className="space-y-4">
               <div>
                 <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-2">{lang === 'en' ? 'Sector' : 'ዘርፍ'}</label>
        ...
        <div className="lg:col-span-3 space-y-8">
           <div className="relative">
             ...
             <input 
               type="text" 
               placeholder={lang === 'en' ? "Search startups..." : "ስታርትአፖችን ይፈልጉ..."}
             ...
             {STARTUPS.map(startup => (
               <div key={startup.id} className="bg-white p-6 rounded-3xl border border-slate-200 hover:shadow-lg transition-shadow">
                 ...
                 <div className="flex gap-2">
                   <button className="flex-1 bg-slate-50 py-2 rounded-lg text-xs font-bold">{lang === 'en' ? 'Details' : 'ዝርዝር'}</button>
